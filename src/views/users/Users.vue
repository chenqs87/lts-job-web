    ...
    <v-toolbar flat color="white">
      ...
      <v-dialog v-model="usersDialog" max-width="500px">
        <template v-slot:activator="{ on }">
          <v-btn
            ...
            class="mb-2"
            v-on="on"
            v-on:click="users.editedIndex = -1"
            >新建用户</v-btn
          >
        </template>
        <v-card>
            ...
            <v-container grid-list-md>
              <v-layout wrap>
                <v-flex xs12 sm6 md12>
                  <v-text-field
                    v-if="users.editedIndex !== -1"
                    disabled
                    v-model="users.editedItem.username"
                    :rules="[
                      () =>
                        !!users.editedItem.username || 'username is required'
                    ]"
                    label="用户名称"
                  ></v-text-field>
                  <v-text-field
                ...
                <v-flex xs12 sm6 md12>
                  <v-select
                    ...
                    :rules="[
                      () => !!users.editedItem.role || 'role is required'
                    ]"
                  ></v-select>
                </v-flex>
                <v-flex xs12 sm6 md12>
                  <v-text-field
                    v-model="users.editedItem.iphone"
                    label="手机号"
                  ></v-text-field>
                </v-flex>
                ...
                <v-flex xs12 sm6 md12>
                  <v-text-field
                    v-model="users.editedItem.email"
                    label="邮箱"
                  ></v-text-field>
                </v-flex>
                ...
                <v-flex xs12 sm6 md12>
                  <v-select
                    ...
                    :rules="[
                      () =>
                        !!users.editedItem.groupName || 'groupName is required'
                    ]"
                  ></v-select>
                </v-flex>
              </v-layout>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="blue darken-1" flat v-on:click="usersDialogClose"
              >Cancel</v-btn
            >
            <v-btn color="blue darken-1" flat v-on:click="saveUsers"
              >Save</v-btn
            >
          </v-card-actions>
        </v-card>
      ...
      <v-dialog v-model="groupDialog" max-width="500px">
        ...
        <v-card>
          ...
          <v-card-text>
            <v-container grid-list-md>
              <v-layout wrap>
                <v-flex xs12 sm6 md12>
                  <v-text-field
                    v-if="groups.editedIndex !== -1"
                    disabled
                    v-model="groups.editedItem.groupName"
                    :rules="[
                      () =>
                        !!groups.editedItem.groupName || 'groupName is required'
                    ]"
                    label="用户组名称"
                  ></v-text-field>
                  <v-text-field
                ...
                <v-flex xs12 sm6 md12>
                  <v-text-field
                    v-model="groups.editedItem.remark"
                    label="描述"
                  ></v-text-field>
                </v-flex>
              </v-layout>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="blue darken-1" flat v-on:click="groupDialogClose"
              >Cancel</v-btn
            >
            <v-btn color="blue darken-1" flat v-on:click="saveGroups"
              >Save</v-btn
            >
          </v-card-actions>
        </v-card>
        ...
        <v-tab-item key="1" value="users">
          <v-card flat>
            <v-data-table
              ...
              :items="users.desserts"
              class="elevation-1"
              :pagination.sync="users.pagination"
              :total-items="users.totalDesserts"
              ...
              <template v-slot:items="props">
                ...
                <td class="justify-center layout px-0">
                  <v-icon
                    small
                    class="mr-2"
                    v-on:click="usersEditItem(props.item)"
                    >edit</v-icon
                  >
                  <v-icon small class="mr-2" v-on:click="delUser(props.item)"
                    >delete</v-icon
                  >
                </td>
              </template>
        ...
        <v-tab-item key="2" value="groups">
          <v-card flat>
            <v-data-table
              ...
              :items="groups.desserts"
              class="elevation-1"
              :pagination.sync="groups.pagination"
              :total-items="groups.totalDesserts"
              :rows-per-page-items="[10, 15, 20, 25, 30]"
            >
              <template v-slot:items="props">
                <td>{{ props.item.groupName }}</td>
                ...
                <td class="justify-center layout px-0">
                  <v-icon
                    small
                    class="mr-2"
                    v-on:click="groupEditItem(props.item)"
                    >edit</v-icon
                  >
                  <v-icon small class="mr-2" v-on:click="delGroup(props.item)"
                    >delete</v-icon
                  >
                </td>
              </template>
...
export default {
  ...
  methods: {
    ...
    queryUsers() {
      let pageNum =
        this.users.pagination.page === null ||
        this.users.pagination.page === undefined
          ? 1
          : this.users.pagination.page;
      let pageSize =
        this.users.pagination.rowsPerPage === null ||
        this.users.pagination.rowsPerPage === undefined
          ? 10
          : this.users.pagination.rowsPerPage;
    ...
    queryGroups() {
      let pageNum =
        this.groups.pagination.page === null ||
        this.groups.pagination.page === undefined
          ? 1
          : this.groups.pagination.page;
      let pageSize =
        this.groups.pagination.rowsPerPage === null ||
        this.groups.pagination.rowsPerPage === undefined
          ? 10
          : this.groups.pagination.rowsPerPage;
