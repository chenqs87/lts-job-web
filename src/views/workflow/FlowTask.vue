  ...
  <div>
      ...
      >
        <template v-slot:items="props">
          <tr @dblclick="props.expanded = !props.expanded">
            <td>
              <a v-on:click="query(props.item)">{{ props.item.id }}</a>
            </td>
            ...
            <td>
              <v-btn small v-if="props.item.status === 0" color="#85ff98"
                >New</v-btn
              >
              <v-btn small v-else-if="props.item.status === 1" color="#f7e155"
                >Pending</v-btn
              >
              <v-btn small v-else-if="props.item.status === 2" color="info"
                >Running</v-btn
              >
              <v-btn small v-else-if="props.item.status === 3" color="#bbb7b7"
                >Killed</v-btn
              >
              <v-btn small v-else-if="props.item.status === 4" color="success"
                >Finished</v-btn
              >
              <v-btn small v-else="props.item.status === 5" color="error"
                >Failed</v-btn
              >
            </td>
            <td class="justify-center layout px-0">
              <v-tooltip top>
                <template v-slot:activator="{ on }">
                  <v-icon
                    ...
                    small
                    class="mr-2"
                    v-on:click="queryScheduleLog(props.item)"
                    >list</v-icon
                  >
                </template>
                <span>作业调度流程</span>
              ...
              <v-tooltip
                top
                v-if="props.item.status === 3 || props.item.status === 5"
              >
                <template v-slot:activator="{ on }">
                  <v-icon
                    v-on="on"
                    small
                    class="mr-2"
                    v-on:click="reTrigger(props.item)"
                    >loop</v-icon
                  >
                </template>
                <span>重新执行</span>
              ...
              <v-tooltip v-if="props.item.status < 3" top>
                <template v-slot:activator="{ on }">
                  <v-icon
                    ...
                    small
                    class="mr-2"
                    v-on:click="killTask(props.item)"
                    >close</v-icon
                  >
                </template>
                <span>Kill工作流</span>
    ...
    <v-dialog v-model="triggerAlertDialog" persistent max-width="1000">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="green darken-1" flat @click="closeTriggerFlowDialog"
            >取消</v-btn
          >
